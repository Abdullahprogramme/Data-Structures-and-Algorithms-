...
def add_airport(graph: dict, city: str, destination: str, weight: int):
    ...
    addEdges(graph, [(city, destination, weight)]) # add the edges to the city
...
def get_secondary_flights(graph: dict, city: str):
    ...
    for connection in connections:
        for edge in graph[connection]:
            if edge[0] not in secondary_flights: # iterate through the edges of the connections and add if not already in the list
...
def BFS_shortest_route(graph: dict, origin: str, destination: str):
    if origin not in graph or destination not in graph:
        return None
    if origin == destination:
        return []
...
def Dijkstra_shortest_route(graph: dict, origin: str, destination: str):
    if origin not in graph or destination not in graph:
        return None
    if origin == destination:
        return []
    # initialize the distances of the cities to infinity
    distances = {city: float('inf') for city in graph}
    distances[origin] = 0
    predecessors = {city: None for city in graph}
    PriorityQueue = [(0, origin)]
    while PriorityQueue:
        cur, city = min(PriorityQueue)
        PriorityQueue.remove((cur, city))
